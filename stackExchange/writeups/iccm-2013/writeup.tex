...
Table \ref{sample-table} includes a description of the model's equations.
...
Stop words are low-predictor words, and \citeauthor listed a short list that were commonly removed (e.g., ``the'', ``and'', ``or'').
...
So instead of explicitly removing an enumerated list of stop words, we used a data-driven approach and let the scaled entropy measure attentuate the words that were not predictive of any tag.
...
The result is a table of co-occurence counts for each of the combinations, which was loaded into R.
...
Our StackOverflow model is a successful case showing that ACT-Rs declarative memory retrieval mechanisms can be scaled up and applied to retrieval requests occuring across large-scale declarative memory datasets. 
The model can successfully tag 67/100 posts correctly, when tasked to predict one tag per post on average.
The model is also computationall space efficient:
Large-scale relational database techniques and efficient sparse matrix data structures were used for implementation,
so that the data structures require only 3GB of memory and retrieval requests with a DM dataset of \num{556677795} chunks occur in less than one second on a single processor.
...
This could potentially introduce authors to tags that are contextually relevant to their interests that they may not have known about.
...
In this scenario the tags for a previously submitted post would be compared againt model predictions.
If the model's predicted tag activation for a used tag is substantially low, then that tag could either be removed from the post, or the post could be flagged for further analysis by an operator.

\subsection{Comparison to \citeA{Kuo2011}}

Our model achieves 67\% classification accuracy when predicting one tag per post on average
(height of full model at vertical dotted line in Figure \ref{fig:ROC})
compared to 47\% accuracy for the co-occurence model presented in \citeA{Kuo2011}.
