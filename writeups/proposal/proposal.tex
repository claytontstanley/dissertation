...
\abstract{
  ...
  The user-customized tag-recommendation models developed with this research should be immediately useful for tag-recommendation systems on the popular Twitter and StackOverflow social media sites.
...
However this may not be the most mathematically appropriate thing to do, since the base-level activation is represented as a log odds value,
and the usual single strength of association term in a random permutation model is a correlation metric.
...
That computation could be made tractable by storing only a small set of values on the order axis and binning.

For example, one could store only three: near, medium, far, which is equivalent to storing three separate co-occurrence matrices.
...
[2] a more thorough evaluation of the strengths, weaknesses, and differences between two cognitively-plausible memory models on a large-scale real-world retrieval task,
...
The tag-recommendation models developed through this research should be immediately relevant for real-world tasks.
If model accuracy is high enough, then the developed models may be directly deployed as tag recommendation systems for either Twitter or StackOverflow.
...
It should be emphasized that it has only recently become possible to evaluate psychological theories of declarative memory (e.g., ACT-R Bayesian, random-permutation vector based)
on large-scale real-world tasks where the theory can be tested on hundreds of millions of data points.
These information-rich large-scale environments provide a unique opportunity to really stress test and explore the impacts of the psychological constraints of each theory on a much larger scale than was previously possible.
...
Discovering an effective way to represent the time decay of information in a vector-based model could improve model fit on other tasks where the memory model has been tested. 
