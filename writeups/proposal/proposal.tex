...
\title{User-customized hashtag/tag prediction on Twitter and StackOverflow:\protect \\ Comparing vector-based and ACT-R memory models of tag retrieval}
\shorttitle{Tag prediction on Twitter and StackOverflow}
%\shorttitle{}
...
\abstract{
  The amount of user-created content on social media sites continues to grow.
  To help organize and structure this information, many users create tags for documents to help connect their content to other posts on similar topics and other users that might have interest.
  If an accurate model of the tags users choose when creating content is available, then the system can recommend tags for a user to use, in order to connect the user to similar topics and people as quickly as possible.
  This works aims to develop and compare tag-recommendation models for users choosing hashtags when composing a tweet on Twitter and choosing tags when asking a question on the StackOverflow site.
  Two cognitively-plausible memory models will be evaluated: an ACT-R inspired Bayesian model and a random-permutation vector-based model.
  Experiments will be aimed to [1] discover the best way to leverage a user's prior hashtag use as a future predictor, [2] improve the models so that each model's strengths are incorporated into the other,
  ...
  This research will focus on developing user-customized models of hashtag prediction, that best balance a user's past hashtag use and the current content in a post being created to predict the chosen hashtags.
  Planned modifications to the two memory models are aimed at the architectural level, and may suggest architectural modifications that generalize to other tasks.
  Finally, the user-customized tag-recommendation models developed with this research may be immediately useful for tag-recommendation systems on the popular Twitter and StackOverflow social media sites.
}
...
I'll also replace the vector-based model used by \textcite{Rutledge2008} with an improved and simplified random-permutation model \parencite{Sahlgren2008}.
...
Nonetheless, it is encouraging that StackExchange finds tag recommendation models beneficial to the user and have started to add them to their question and answer sites. 
...
The SNIF-ACT framework \parencites{Fu2007,Pirolli2003} is also based on the large-scale version of $S_{ji}$, and scales well when working with a large-scale search query database for internet search.
...
``zend'' has co-occurred with \emph{PHP}, so the unordered memory vector for ``zend'' ($c_{zend}$) contains the unordered environment vector for \emph{PHP} ($e_{\mathit{PHP}}$).
...
So the cosine between the environment vector \emph{PHP} ($e_{\mathit{PHP}}$) and decoded memory vector for ``zend'' ($c_{zend}$) will be high, and most likely \emph{PHP} will be retrieved through decoding.
...
Retrieval accuracy for the random-permutation vector-based models remained high even after reducing each environment vector's length from \num{2048} to \num{1024}.
