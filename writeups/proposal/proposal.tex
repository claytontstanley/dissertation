...
The random permutation model described in \textcite{Sahlgren2008} will be used for the vector-based model, since it has shown to perform better than the BEAGLE model \parencite{Recchia2010}.
The modified ACT-R Bayesian model described in \textcite{Stanley2013} will be used for the Bayesian model, since it has already been applied to tag use on StackOverflow posts.

\subsubsection{ACT-R Bayesian Model}
...
\begin{table}[!ht]
  ...
  {\tabulinesep=1.2mm
    \begin{tabu}{ll}
      ...
      Memory Vector &		$m_{i} = e_{i} + \sum_{j \in all past} c_{j} + \sum_{j \in all past} \sum_{k \in locations} o_{j,k}$ \\
      Unordered Context &	$c_{i} = e_{i}$ \\
      Ordered Context &		$o_{i,k} = e_{i^{-k}}$ \\
      Context Memory Vector &	$m_{C} = \sum_{j \in C} c_{j} + \sum_{j \in C} \sum_{j \in locations} o_{j,k}$ \\
