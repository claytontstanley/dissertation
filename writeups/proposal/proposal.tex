...
The change to incorporate user's priors was to compute a weighted average for $p_{i}$,
where the probability of using tag $i$ would range from the global probability ($pGlobal_{i}$) to the user's specific probability ($pUser_{i}$) as the user's total tag count ($count$) increases.
