...
In order to represent word order information, a funciton must be used that converts the memory vectors for a set of words (``stack $\Phi$'') into a new uncorrelated environment vector.
The BEAGLE model uses circular convolution and inverse correlation as the encoding and decoding operations.
Howerver, that is certainly not the only operation that can generate new uncorrelated vectors from a set of memory vectors.
\textcite{Sahlgren2008} used a much simpler random permutation method for this operation.

Random permutatinos use a large sparse vector of zeros with a few ones in random locations as an environment vector for each of the words.
In order to create a new vector for a particular word that occurs one back ($\mathit{stack}^{-1}$), that word's environment vector ($e_{stack}$) is bit shifted to the left one position.
This produces a new environment vector for the combination of ``stack'' and the one back position that is uncorrelated with the original ``stack'' environment vector and all other environment vectors.
So if ``stack overflow'' occurred in a sentence, the one-back environment vector for ``stack'' would be added to the memory vector for ``overflow'' ($m_{overflow} = m_{overflow'} + e_{stack^{-1}}$).
...
The memory vector for ``overflow'' will be correlated the highest with this context vector since it contains this vector.

\subsubsection{Comparison between BEAGLE and Random Permutations}

RP used the simplest permutation possible to create a new environment vector.
Performance is actually better than BEAGLE.
Computational efficiency is better as well.
Strong case for using RP over circular convulation for decoding / encoding.
